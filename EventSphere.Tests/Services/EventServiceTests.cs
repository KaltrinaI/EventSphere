...
namespace EventSphere.Tests.Services
{
    public class EventServiceTests
    {
        ...
        public async Task UpdateEvent_ShouldCallRepositoryUpdateEvent_WhenEventDtoIsValid()
        {
            // Arrange
            var eventId = 1;
            var eventDto = new EventRequestDTO { Name = "Updated Event", Description = "Updated Description", StartDate = DateTime.Now, EndDate = DateTime.Now.AddDays(1), Location = "Updated Location", Capacity = 100, OrganizerId = 1 };
            ...
            // Assert
            _repositoryMock.Verify(r => r.GetEventById(eventId), Times.Once);
            _repositoryMock.Verify(r => r.UpdateEvent(eventDto, eventId), Times.Once);
        }
        ...
        public async Task UpdateEvent_ShouldPropagateException_WhenRepositoryThrowsException()
        {
            ...
            _repositoryMock.Setup(r => r.GetEventById(eventId)).ReturnsAsync(existingEvent);
            _repositoryMock.Setup(r => r.UpdateEvent(eventDto, eventId)).ThrowsAsync(new Exception("Database error"));
